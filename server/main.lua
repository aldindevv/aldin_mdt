...
MySQL.ready(function()
    local result = MySQL.Sync.fetchAll("SELECT * FROM mdt_general")
...
AddEventHandler("fizzfau-mdt:server:search-citizen", function(result)
    local data = false
    local src = source
    local result = MySQL.Sync.fetchAll("SELECT * FROM users WHERE CONCAT(firstname, ' ', lastname) LIKE '%"..result.."%' OR phone_number LIKE '%"..result.."%'")
    if result[1] then
        data = result
    end
    for i =1, #result do
...
AddEventHandler("fizzfau-mdt:server:search-vehicle", function(result)
    local data = false
    local src = source
    local results = MySQL.Sync.fetchAll("SELECT * FROM owned_vehicles WHERE plate LIKE '%"..result:upper().."%' OR modelname LIKE '%"..result.."%'")
    if results[1] ~= nil then
        data = results
        for i =1, #data do
            local result2 = MySQL.Sync.fetchAll("SELECT * FROM users WHERE identifier LIKE '%"..data[i].owner.."%'")
...
AddEventHandler("fizzfau-mdt:server:server:getInfo", function(identifier)
    ...
    local results = MySQL.Sync.fetchAll("SELECT info, bolos FROM mdt_warrants WHERE identifier = '" ..identifier.. "'")
    local vehicles = MySQL.Sync.fetchAll("SELECT plate, modelname FROM owned_vehicles WHERE owner = '" ..identifier.. "'")
    if results[1] ~= nil then
        ...
        for k,v in pairs(MDT.Infos[data.identifier]) do
            ...
            if tonumber(k) == id then
                ...
                if (MDT.Infos[data.identifier] == nil and MDT.Bolos[data.identifier] == nil) or retval then
                    ...
                    MySQL.Async.execute("DELETE FROM mdt_warrants WHERE identifier = '" ..data.identifier.. "'")
                else
                    MySQL.Async.execute("UPDATE mdt_warrants SET info = @notes WHERE identifier = '" ..data.identifier.. "'", {
                        ["@notes"] = notes
                    })
                end
        ...
        for k,v in pairs(MDT.Bolos[data.identifier]) do
            ...
            if tonumber(k) == id then
                ...
                if (MDT.Infos[data.identifier] == nil and MDT.Bolos[data.identifier] == nil) or retval then
                    ...
                    MySQL.Async.execute("UPDATE mdt_warrants SET bolos = @bolos WHERE identifier = '" ..data.identifier.. "'", {
                        ["@bolos"] = bolos
                    })
                end
...
AddEventHandler("fizzfau-mdt:server:add-note", function(data, myName)
    ...
    if (MDT.Infos[data.identifier]) == nil and (MDT.Bolos[data.identifier]) == nil  then
        ...
        if MDT.Infos[data.identifier][id] == nil then
            ...
            MySQL.Async.execute("INSERT INTO mdt_warrants (identifier, info) VALUES (@identifier, @info)", {
...
AddEventHandler("fizzfau-mdt:server:add-bolo", function(data, myName)
    ...
    if (MDT.Infos[data.identifier]) == nil and (MDT.Bolos[data.identifier]) == nil  then
        ...
        if MDT.Bolos[data.identifier][id] == nil then
            ...
            MySQL.Async.execute("INSERT INTO mdt_warrants (`identifier`, `bolos`) VALUES (@identifier, @bolos)", {
                ["@identifier"] = data.identifier,
                ["@bolos"] = json.encode(MDT.Bolos[data.identifier])
            })
    ...
    if image ~= "" then
        MySQL.Async.execute("UPDATE users SET mdt_image = @image WHERE identifier = '" ..data.identifier.. "'", {
            ["@image"] = image
        })
    end
    TriggerClientEvent("fizzfau-mdt:server:client:getInfo", src, info, data.identifier)
    ...
    if resource == "fizzfau-mdt" then
        if MDT.Actions ~= nil then
            if #MDT.Actions > 0 then
                MySQL.Async.execute("UPDATE mdt_general SET actions = @actions", {
...
AddEventHandler("fizzfau-mdt:server:add-wanted", function(data, myName)
    ...
    MySQL.Async.execute("INSERT INTO mdt_wanteds (wanted) VALUES (@wanted)", {
        ["@wanted"] = json.encode(data)
    })
    local action = {type = "Created Wanted", time = time, name = myName}
    AddAction(action)
    DiscordLog({title = "Wanted - " ..data.title, text = " \n" ..data.text, image = image}, myName, "wanted")
    TriggerClientEvent("fizzfau-mdt:client:notify", -1, "Officer " ..myName.. " has created a wanted record! ")
end)    

ESX.RegisterServerCallback("fizzfau-mdt:getWanteds", function(source, cb)
    local wanteds = {}
    local result = MySQL.Sync.fetchAll("SELECT * FROM mdt_wanteds")
    if result ~= nil then
        for i =1, #result do
...
AddEventHandler("fizzfau-mdt:server:delete-wanted", function(data, myName)
    ...
    MySQL.Async.execute("DELETE FROM mdt_wanteds WHERE id = @id", {
        ["@id"] = id
    })
end)   
